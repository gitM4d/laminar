 ...
 * Yield Focused profile (minTrustScore 65) but is intentionally below Balanced
 * (75), so a yield-focused intent demonstrates real strategy allocations.
 */
const DEFAULT_INTENT = { risk: 8, liquidity: 3, returnPreference: 9 };
const DEFAULT_PORTFOLIO_USD = 10_000;

async function main(): Promise<void> {
  console.log("Laminar — Moonwell Base Real Provider Recommendation");
  console.log("====================================================");
  ...
  console.log(
    "- Default API/frontend provider remains MockLaminarDataProvider.",
  );
}

main().catch((error: unknown) => {
  console.error("Moonwell recommendation probe failed:");
  console.error(error);
  process.exitCode = 1;
});
